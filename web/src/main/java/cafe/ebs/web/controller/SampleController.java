...
public class SampleController {
	...
	public String addSample(@RequestParam(value="sampleName") String sampleName) {
		...
		// 잘 받아왔는지 확인
		System.out.println("등록 할 sampleName :" + sampleName);
		...
		// 결과 확인
		System.out.println("입력 결과(1:성공/0:실패):"+result);
		// 회원 리스트로 이동하기
	...
	public String deleteSample(@RequestParam(value="sampleId") int sampleId) {
		// 삭제 액션에 왔는지 확인한다.
		System.out.println("Request : /deleteSample : 삭제 액션");
		// 잘 받아왔는지 확인
		System.out.println("삭제 할 sampleId :" + sampleId);
		// int타입의 sampleId를 vo의 Sample에 있는 sampleId에 set,get을 위한 sample참조변수에 setting함
		// sample객체에 삭제할 id를 세팅한다.
		sample.setSampleId(sampleId);
		// sample참조변수를 Service의 deleteSample매서드를 통해 삭제 액션을 처리, 그 결과값을 int타입의 result변수에 담는다.
		int result = sampleService.deleteSample(sample);
		// 결과가 성공적이면 1, 그렇지 못하면 0으로 출력된다.
		System.out.println("삭제 결과(1:성공/0:실패):"+result);
		// 삭제 처리가 되었다면 전체 리스트로 이동한다.
		return "redirect:/sampleList";
	}
	// 5. 수정 폼
	...
	public String updateSample(@RequestParam(value="sampleId") int sampleId, Model model) {
		...
		// 잘 받아왔는지 확인
		System.out.println("수정 할 sampleId :" + sampleId);
